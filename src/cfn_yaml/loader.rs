...
use crate::cfn_yaml::{
    errors::{Error, InternalError::InvalidKeyType, Result},
    event::{Event, Scalar, ScalarStyle, SequenceStart},
    mappings::{short_form_to_long, SEQUENCE_VALUE_FUNC_REF, SINGLE_VALUE_FUNC_REF},
    parser::Parser,
    types::{Location, MarkedValue},
};
...
impl Loader {
    ...
    pub(crate) fn load(&mut self, content: String) -> Result<MarkedValue> {
    ...
    fn handle_mapping_end(&mut self) -> Result<()> {
